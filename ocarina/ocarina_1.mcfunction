...
execute unless entity @a[tag=OcarinaMinigame] if entity @s[tag=Lullaby] if entity @a[tag=ForestMedallion] run summon minecraft:armor_stand ^ ^-0.5 ^ {Tags:["Ocarina","TimeExtendet"],Invisible:1b,NoGravity:1,Rotation:[120f],ArmorItems:[{},{},{},{id:tnt_minecart,Count:1,tag:{OcarinaSong:1b,display:{Name:"{\"text\":\"Extended Song of Time\"}"}}}],Pose:{Head:[-90f,180f,0f]}}
...
execute as @e[type=armor_stand,tag=TimeExtendet,tag=Ocarina] at @a rotated ~ 0 run tp @s ^ ^-.5 ^ ~ 0
#execute as @e[type=armor_stand,tag=TimeExtendet,tag=Ocarina] at @a rotated ~ 0 run tp @s ^ ^1 ^ 
